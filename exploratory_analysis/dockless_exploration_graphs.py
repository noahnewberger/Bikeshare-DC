...
if __name__ == '__main__':
    ...
    pct_df['dless_tot'] = pct_df['dless_trips_all']/(pct_df['dless_trips_all'] + pct_df['cabi_trips_wdc_to_wdc']) * 100
    pct_df['cabi_tot'] = pct_df['cabi_trips_wdc_to_wdc']/(pct_df['dless_trips_all'] + pct_df['cabi_trips_wdc_to_wdc']) * 100
    pct_df['dless_casual'] = pct_df['dless_trips_all']/(pct_df['dless_trips_all'] + pct_df['cabi_trips_wdc_to_wdc_casual']) * 100
    pct_df['cabi_casual'] = pct_df['cabi_trips_wdc_to_wdc_casual']/(pct_df['dless_trips_all'] + pct_df['cabi_trips_wdc_to_wdc_casual'])* 100
    ...
    width = 0.35
    pct_df.plot.bar(x='index', y=dless_pct, ax=axes, stacked=True)
    axes.set_xticklabels((
        'September', 'October', 'November', 'December', 'January', 'Feburary',
        'March', 'April'), rotation=0)
    axes.set_xlabel('Month')
    axes.legend(['Jump', 'Lime', 'Mobike', 'Ofo', 'Spin'])
    all_in_one_save(
            "Market Share Dockless", "C:/Users/Noah/Bikeshare-DC_Old/For Upload", dr,
            '1LRJWj6wLBWvyBJbN93jXA2dpgF3BLrN3')
